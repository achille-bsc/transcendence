...
async function authPlugin(server: FastifyInstance) {
  ...
  server.decorate('authenticate', async (request: FastifyRequest, reply: FastifyReply) => { // 'décore' mon serveur d'une fonction 'authenticate', pour qu'elle soit partout, je l'utilise pour vérifier le token à chaque route protégée
    try {
